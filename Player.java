...
/*	public String name;
	public Scanner scan;
	public List<IndCards> hand = new ArrayList<IndCards>(), stable = new ArrayList<IndCards>(),
			affectCards = new ArrayList<IndCards>();
	public int playerOrder, drawNum, playNum, maxStableCards;
	public boolean bringBasic, bringBaby, blockable, discarded, destroyable, showHand, barbedWire, playEffect, 
		unicornEffect, differentDeath;
	...
	public void setOrder(int babyRank) {
		playerOrder = babyRank;
	}
	
	public void addTohand(IndCards getCard) {
		hand.add(getCard);
	}
	
	public void playUnicorn(IndCards getUnicorn) {
		stable.add(getUnicorn);
	...
	public void addAffectCard(IndCards getAffect) {
		affectCards.add(getAffect);
	...
	public List<IndCards> getAffectCard() {
		return affectCards;
	}
	...
	public boolean checkForCard(String targetCard, List<IndCards> placement) {
		for (IndCards checking: placement) {
			if (checking.getName().equals(targetCard)) {
				return true;
			}
		}
		return false;
	...
	public boolean checkForBeginning (){
		for (IndCards checking: stable) {
			if (checking.getTiming() == 1) {
				return true;
			}
		}
		for (IndCards checking: affectCards) {
			if (checking.getTiming() == 1) {
				return true;
			}
		}
	...
	public boolean playerWin() {
		if (checkForCard("Pandamonia", affectCards)) {
			return false;
		}else if (stable.size()>= 7 || (stable.size() >= 6 && checkForCard("Ginormous Unicorn", stable)) ) {
			return true;
		}
		return false;
	}
	
	public void addToHand (IndCards additionalCard) {
